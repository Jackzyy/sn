...
let fileName = ''
const goodsPageUrl =
  'https://mpm.suning.com/mpms-web/sp/shopcoupon/goPromotionSummaryPage.do'
const goodsDetailUrl =
  'http://mpm.suning.com/mpms-web/sp/shopcoupon/goDetailImportPage.do'
...
const utils = {
  ...
  async getCurrentPageGoodsId(sTime, eTime, currentPage) {
    ...
    tableData.each((index, item) => {
      ...
      let goodsItemTime = $($(item).find('td')[3]).text().replace(/\s+/g, '')
      // 门槛（满减）
  ...
  async getGoodsDetail(goodsInfo) {
    ...
    $($('.active-detail').find('li')[4]).each((index, item) => {
      $(item)
        .find('i')
        .each((index, item) => {
          if (index === 1) {
    ...
    let len = $('.infoTable').find('tbody').find('tr').length
    $('.infoTable')
      .find('tbody')
      .find('tr')
      .each((index, item) => {
        ...
        if (len === 5) {
          ...
          if (index === 3) {
            $(item)
              ...
              .each((index, item) => {
                if (index === 1) {
                  i = $(item).find('span').text().replace(/\s+/g, '')
                }
              })
          }
